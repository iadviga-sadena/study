...
class Person:
    ...
    def __eq__(self, obj):
        if type(obj) == Person:
            ...
            return self.__fuzzy_compare(obj) > 0.49
        else:
    ...
    def __fuzzy_compare(self, query):
        ...
        return score  # возвращает числовой результат вместо булевого

    def score(self, query):  # для получения значения score
        return self.__fuzzy_compare(query)
...
# Вывод отладочной информации: запрос, объект, результат сравнения
'''for query, person in product(queries, people.values()):
    if person == query:
        pprint(((query, person), person.score(query)))
    else:
        print(('Неподходящий вариант', (query, person), person.score(query)))'''

# Вывод результатов + ошибочных запросов
for query in queries:
    ...
    for person in people.values():
        if person == query:
            ...
            count += 1
    if count == 0:
        print('Ничего не найдено по следующему запросу ==> "{}"'.format(query))